...
<td class="td" width="10%"><a href="chukuupdate.jsp?keyid=<%=rs.getString("ckid")%>">编辑</a> |<a href="chukudelete.jsp?keyid=<%=rs.getString("ckid")%>"   onClick="javascript:if(confirm('是否删除')){return true;}else{return false;}">删除</a>|<a href="chukudetail.jsp?keyid=<%=rs.getString("ckid")%>">查看</a></td>
</tr>
 ...
 <td align="right">当前页数：[<%=Page%>/<%=maxPage%>]&nbsp;
	<%if(Page>1){%>
<a href="chukulist.jsp?Page=1">第一页</a>　<a href="chukulist.jsp?Page=<%=Page-1%>">上一页</a>
	<% }
if(Page<maxPage){%>
<a href="chukulist.jsp?Page=<%=Page+1%>">下一页</a>　<a href="chukulist.jsp?Page=<%=maxPage%>">最后一页&nbsp;</a>
	<%}
